...
# todo:
# - compute sleep efficiency (% asleep out of total time spent in bed)
...
### Chart Notes
  ...
  * Night Review
    * Sleep Quality
      - **How I Subjectively Rated Each Night**: this is how I rated each night. Note that a normal person's 5/10 rating is probably my 8/10 rating
    ...
    * Efficiency
      - **Sleep Efficiency**: I believe this metric is often used in clinical studies to determine one's quality of sleep. The most important thing about it, is that it can be influenced by me to a degree, for sleep restriction for example (as promoted by CBT for Insomnia).
    * Sleep Debt
      - **Sleep Debt**: In a book I read "Overcoming Insomnia and Sleep Problems", the author mentioned that every night we get less sleep than we need, we accumulate "sleep debt" and our body will be looking to catch up on the lost sleep the following night. It said that for every 1 hour of lost sleep (sleep below what you need or what you are getting on average) you only need 1/3 of extra hour the following night to recover -- this plot attempts to demonstrate these dynamics.
  * Influencing Factors:
    * **Alcohol Consumtion**: These plots show how much alcohol I consumed daily and over last N days. The values are expressed in terms of standard drinks and are approximate.
  * Analytics:
...
summary_p <- summary_plot(sleep_collection)
sleeping_pills_p <- sleeping_pill_info(sleep_collection)
g <- plot_grid(
  summary_p,
  sleeping_pills_p, 
  ncol=1, 
...
g <- plot_grid(
  p_over_time + theme(axis.text.x = element_blank()),
  p_hours_of_sleep, 
  ncol=1, 
  rel_heights=c(6,6), 
  align="v"
...
### Sleep Debt
```{r sleep_debt, fig.width=12, fig.height=7.65}
damage_plot(sleep_collection)
```

### Sleep Efficiency
```{r sleep_efficiency, fig.width=12, fig.height=7.65}
sleep_efficiency_plot(sleep_collection)
```
...
```{r daytime_energy, fig.width=12, fig.height=7.65}
daytime_energy_plot(sleep_collection)
```

### Condition

```{r condition, fig.width=12, fig.height=7.65}
condition_plot(sleep_collection)
```


# Influencing Factors

```{r alcohol_consumption, fig.width=12, fig.height=7.65}
p_daily_alcohol <- daily_alcohol_plot(sleep_collection)
p_agg_alcohol <- alcohol_plot_over_range(sleep_collection)

g <- plot_grid(
  p_agg_alcohol + theme(axis.text.x = element_blank()),
  p_daily_alcohol, 
  ncol=1, 
  rel_heights=c(6,6), 
  align="v"
