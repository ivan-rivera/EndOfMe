...
hours_of_sleep <- function(sleep_collection){
  p <- sleep_collection[["sleep stats"]] %>% 
    ...
    geom_bar(aes(y=value, fill=variable), alpha=1, stat="identity") +
    ...
    ggtitle(
      ...
      subtitle = sprintf("dashed lines are %s day averages", days_to_average)
    ) +
    scale_x_date(
    ...
    scale_fill_manual("", values=c(
      "time asleep" = custom_color_palette[["primary"]][4], 
...
falling_asleep_and_staying_awake <- function(sleep_collection){
  p <- sleep_collection[["sleep stats"]] %>% 
    ...
    ggtitle(
      ...
      subtitle=sprintf("dashed line represents a %s day average", days_to_average)
    ) + 
    scale_x_date(
      date_breaks=sprintf("%s days", n_day_breaks), 
      expand=c(expand_settings[1], expand_settings[2]+0.25)
...
damage_plot <- function(sleep_collection){
  p <- sleep_collection[["sleep"]] %>%
    ...
    geom_hline(yintercept = 0, linetype="dotted", color=custom_color_palette[["primary"]][1]) + 
